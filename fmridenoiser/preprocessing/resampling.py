...
def check_geometric_consistency(
    ...
    if reference_file is not None:
        ...
        logger.debug(f"Reference geometry from {reference_file.name}: shape={reference_shape}")
    else:
        ref_img = nib.load(func_files[0])
        reference_shape = ref_img.shape[:3]
        reference_affine = np.round(ref_img.affine, decimals=6)
        logger.debug(f"Reference geometry from {func_files[0].name}: shape={reference_shape}")
    ...
    for func_file in func_files:
        ...
        geometries[str(func_file)] = {'shape': shape, 'affine': affine}
    ...
    for file_path, geom in geometries.items():
        ...
        if not np.array_equal(geom['shape'], reference_shape):
            logger.warning(f"Shape mismatch: {file_name} has {geom['shape']} vs reference {reference_shape}")
            mismatches.append(file_name)
            is_consistent = False
        if not np.allclose(geom['affine'], reference_affine, rtol=1e-5):
            logger.warning(f"Affine mismatch: {file_name} differs from reference")
    ...
    if is_consistent:
        ...
        logger.warning(f"Geometric inconsistencies detected in {len(mismatches)} image(s) - resampling required")
